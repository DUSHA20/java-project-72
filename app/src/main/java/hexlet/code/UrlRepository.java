...
public class UrlRepository extends BaseRepository {
    ...
    public void initializeDatabase() {
        try (Statement statement = connection.createStatement()) {
            ...
            String createUrlsChecksTableSql = "CREATE TABLE IF NOT EXISTS urls_checks (" +
                    "id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY," +
                    ...
                    ")";
            statement.executeUpdate(createUrlsChecksTableSql);

            System.out.println("Tables 'urls' and 'urls_checks' created successfully");
    ...
    public void addUrlCheck(String name_url, String title, String h1, String description) {
        try {
            PreparedStatement statement = connection.prepareStatement("INSERT INTO urls_checks (name_url, title, h1, description, created_at) VALUES (?, ?, ?, ?, ?)");
    ...
    public List<UrlCheck> getUrlChecksByUrl(String url) {
        ...
        try {
            PreparedStatement statement = connection.prepareStatement("SELECT * FROM urls_checks WHERE url = ?");
            statement.setString(1, url);
            ResultSet resultSet = statement.executeQuery();
            while (resultSet.next()) {
    ...
    public List<UrlCheck> getAllUrlChecks() {
        List<UrlCheck> urlChecks = new ArrayList<>();
        String sql = "SELECT * FROM urls_checks"; // Получаем все данные из таблицы url_checks
        try (PreparedStatement statement = connection.prepareStatement(sql);
             ResultSet resultSet = statement.executeQuery()) {
            while (resultSet.next()) {
                UrlCheck urlCheck = new UrlCheck();
