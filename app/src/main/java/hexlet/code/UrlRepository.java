...
public class UrlRepository extends BaseRepository {
    ...
    public void initializeDatabase() {
        try (Statement statement = connection.createStatement()) {
            ...
            String createLinkCheckTableSql = "CREATE TABLE IF NOT EXISTS LinkCheck (" +
                    "id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY," +
                    "url_id BIGINT," + // Новый столбец
                    "url VARCHAR(255) NOT NULL," + // Столбец url
    ...
    public void saveUrlStatus(String url, int statusCode, LinkCheck.LinkType linkType) throws SQLException {
        ...
        if (urlId != null) {
            try {
                PreparedStatement statement = connection.prepareStatement("INSERT INTO LinkCheck (url_id, url, statusCode, checkedAt, linkType) VALUES (?, ?, ?, ?)");
                statement.setLong(1, urlId);
                statement.setString(2, url);
                statement.setInt(3, statusCode);
                statement.setTimestamp(4, Timestamp.valueOf(LocalDateTime.now()));
                statement.setString(5, linkType.name());
                statement.executeUpdate();
            } catch (SQLException e) {
                e.printStackTrace();
    ...
    public List<LinkCheck> getAllLinkChecks() {
        ...
        String sql = "SELECT id, url_id, url, statusCode, checkedAt, linkType FROM LinkCheck";
        try (PreparedStatement statement = connection.prepareStatement(sql);
             ResultSet resultSet = statement.executeQuery()) {
            while (resultSet.next()) {
